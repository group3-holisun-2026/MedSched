...
export default function CalendarPage() {
  ...
  // Citeste mesajul de eroare din raspunsul serverului indiferent de shape: unele exceptii (ex.
  // ResourceConflictException, Modulul 3) intorc inca text brut in body, in timp ce restul erorilor
  // (inclusiv noile tranzitii de status respinse de AppointmentStateMachine, care folosesc
  // ResponseStatusException) trec prin GlobalExceptionHandler si ajung ca ErrorResponse standard cu
  // camp `message`. Verificam ambele forme ca sa nu ajungem cu "undefined" in toast pentru un caz
  // specific de backend, la fel ca bug-ul deja cunoscut din AppointmentForm.jsx.
  ...
  // Rulaza o tranzitie de status pe programarea selectata, cu stare de "se proceseaza", toast de
  // reusita/eroare si tratare speciala pentru 409 (tranzitie invalida / altcineva a actionat deja
  // din alta sesiune): afisam mesajul de conflict venit din server SI reimprospatam automat
  // calendarul, ca userul sa vada starea reala curenta, nu doar un toast izolat care lasa UI-ul
  // desincronizat pana la urmatorul ciclu de polling (20s).
      ...
      >
        {selectedEvent && (
          <div>
            ...
            {/*
                          Panou de actiuni, conditionat de statusul curent (conform mașinii de stări
                          din backend_module4_tasks.md §1.1). Vizibilitatea suplimentara pe ROL
                          (ex. doar ADMIN/RECEPTION vad Confirma/Neprezentat, DOCTOR vede doar
                          check-in pe propria programare) e un item separat de task - nu o
                          implementam aici. Nu e o gaura de securitate: backend-ul respinge oricum
                          cu 403 orice actiune neautorizata, indiferent ce arata UI-ul.
                        */}
